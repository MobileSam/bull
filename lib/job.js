...
/**
*/
Job.fromData = function(queue, jobId, jobData){
  var data;
  try{
    data = JSON.parse(jobData.data);
  } catch (err) {
    //swallow exception because data got corrupted somehow.
    data = {};
  }
  ...
  try{
    ...
    //swallow exception because opts got corrupted somehow.
    opts = {};
  }

  var job = new Job(queue, data, opts);
  job.jobId = jobId;
  // The following values might be NaN if corrupted
  job._progress = parseInt(jobData.progress);
  job.delay = parseInt(jobData.delay);
  job.timestamp = parseInt(jobData.timestamp);
  job.attempts = parseInt(jobData.attempts);
  if(isNaN(job.attempts)) {
    job.attempts = 1; // Default to 1 try for legacy jobs
  }
  job.attemptsMade = parseInt(jobData.attemptsMade);
  var _traces;
  try{
    _traces = JSON.parse(jobData.stacktrace);
  ...
  try{
    job.returnvalue = JSON.parse(jobData.returnvalue);
  }catch (e){
